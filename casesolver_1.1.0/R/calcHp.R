...
calcHp = function(evidData,refData,nC,popFreq,kit,pC,lambda,threshT,xi,nDone=4) { 
  uselocs <- names(evidData[[1]]) #use loci given in data
  ...
  if(nC>2) fitHp <- contLikMLEpara(nC,evidData,data$popFreq,data$refData,condOrder=condOrder,xi=xi,prC=pC,lambda=lambda,nDone=nDone,threshT=threshT,kit=kit,verbose=FALSE)
  if(nC<=2) fitHp <- contLikMLE(nC,evidData,data$popFreq,data$refData,condOrder=condOrder,xi=xi,prC=pC,lambda=lambda,nDone=nDone,threshT=threshT,kit=kit,verbose=FALSE)
