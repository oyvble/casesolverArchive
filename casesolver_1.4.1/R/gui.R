...
#NB: pgkPath and .sep must be changed before compiling R-package!
 ...
 if(!file.exists(setupFileAdvanced)) {  #use default values if not existing
   setupAdvanced = list(maxC1=4,maxC2=3,nDone=2,pD0=0.1,useMinK1="TRUE")
 } else {
   optF <- setupRead(file=setupFileAdvanced)
   setupAdvanced = list(maxC1=as.integer(optF[1]),maxC2=as.integer(optF[2]),nDone=as.integer(optF[3]),pD0=as.numeric(optF[4]),useMinK1=optF[5])
 }
 ...
 f_importsel=  function(h,...) {
  ...
  f_options = function(h,...) { 
   ...
   tabval[5,1] <- gWidgets2::gbutton("Save", container=tabval,handler = function(h, ...) { 
    ...
    opt$useMinK1 <- as.character(gWidgets2::svalue(tabval[4,2])==TRUE)  #should K=1 be used as start contr?
...
# gWidgets2::gedit(text=nC,container=tabSel)
 ...
 showMatchNetwork = function(h,...) {
   ...
   score <- as.numeric(tab[,4])
   keep <- (type=="qual" & score>=log10(threshLR1)) | (type=="quan" & score>=log10(threshLR2))
   ...
   tab2 <-  data.frame(from=tab[,2],to=tab[,1],weight=sqrt(as.numeric(tab[,4])),nC=as.integer(tab[,5]))
   ...
   cols[names(nods)%in%tab2$to[tab2$nC==1]] <- "cyan" #assigned as single sources (becomes blue)
  ...
  suppressWarnings({
   ...
   if(type=="quan") {
	 ...
	 xi0 = 0 #no stutters assumed by default
      ...
      matchLRres <- calcQuanLRcomparison(DBmix,DBref,matchlist=Clist[,1:3],popFreq=get("popFreq",envir=nnTK),kit=kit0,xi=xi0,pC=get("setupModel",envir=nnTK)$dropinC,lambda=get("setupModel",envir=nnTK)$dropinL,threshT=get("setupModel",envir=nnTK)$threshT,nDone=get("setupAdvanced",envir=nnTK)$nDone,maxC=get("setupAdvanced",envir=nnTK)$maxC2,nContr=ncontr) 
   }
  })
...
################################################################################################
 ...
 tabCompA3[1,4] <- gWidgets2::gcombobox(items=sortMixListTablesTxt,selected=1,container=tabCompA3,handler=
   function(h,...) {
     refreshTabLIST(sort=which(svalue(tabCompA3[1,4])==sortMixListTablesTxt)) #change sorted order
   }
 )  
