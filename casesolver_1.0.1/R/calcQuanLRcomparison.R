...
calcQuanLRcomparison = function(DBmix,DBref,matchlist,popFreq,kit,pC,lambda,threshT,nDone=4,maxC=3) { 
 require(euroformix) 
...
#    if(length(x)==1) x <- rep(x,2)
 ...
 systime <- system.time( {
  ...
  data <- euroformix::Qassignate(sample, popFreq[uselocs],incS=FALSE) #don't include stutters, use all loci
  ...
  print(paste0("Assumed number of contributors=",nC))
  
  #Calc Lik(E|Hd): 
  if(nC>2) fitHd <- euroformix::contLikMLEpara(nC,sample,data$popFreq,xi=0,prC=pC,lambda=lambda,nDone=nDone,threshT=threshT,kit=kit,verbose=FALSE)
  if(nC<=2) fitHd <- euroformix::contLikMLE(nC,sample,data$popFreq,xi=0,prC=pC,lambda=lambda,nDone=nDone,threshT=threshT,kit=kit,verbose=FALSE)
 #NOTICE: These values can be stored for later for deconvolution
  #fitHd2 <- contLikMLE(nC+1,sample,data$popFreq,xi=0,prC=pC,lambda=lambda,nDone=1,threshT=threshT,kit=kit,verbose=FALSE)
  #fitHd$fit$thetahat2
  #fitHd2$fit$thetahat2
  ...
  for(rr in whatR ) { #for each references
   #rr=whatR[1]
   if(!rr%in%names(DBref)) { #if reference not found in dataList, we will create list:
    refD <- lapply(DBmix[rr],function(x) x[uselocs]) #extract reference
   } else {
    refD <- lapply(DBref[rr],function(x) x[uselocs]) #extract reference
   } 
   ...
   data2 <- Qassignate(sample, popFreq[uselocs],refD2,incS=FALSE,incR=FALSE) #popFreq must be given with correct order?

   #Calc Lik(E|Hp)
   if(nC>2) fitHp <- contLikMLEpara(nC,sample,data2$popFreq,data2$refData,condOrder=1,xi=0,prC=pC,lambda=lambda,nDone=nDone,threshT=threshT,kit=kit,verbose=FALSE)
   if(nC<=2) fitHp <- contLikMLE(nC,sample,data2$popFreq,data2$refData,condOrder=1,xi=0,prC=pC,lambda=lambda,nDone=nDone,threshT=threshT,kit=kit,verbose=FALSE)
